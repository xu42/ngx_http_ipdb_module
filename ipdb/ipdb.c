...
int ipdb_reader_new(const char *file, ipdb_reader **reader) {
    ...
    *reader = malloc(sizeof(ipdb_reader));
    ...
    size_t result3 = fread(rd->data, sizeof(unsigned char), (size_t) data_len, fd);
    if (result3 > 0) {}
